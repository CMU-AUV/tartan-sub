...
class circle(object):
    ...
    rod_s = cv2.imread('/home/roboclub/catkin_ws/src/qualify_task/rod_img_small.png', 0)
    ...
    def edge_detecting(self, src):
        ...
        if len(bar) > 0 and len(legs) > 0:
            ...
            bar = zip(*bar)
            y_list = bar[0]
            y_list = [y for y in y_list if y < y_list[0]+40]
    ...
    def toPict(self, X_p, X):
        w = (X_p[1]-X[1])*math.cos(X[3]) - (X_p[0]-X[0])*math.sin(X[3])
    ...
    def search4Gate(self):
        rad2turn = math.pi-self.theta_mag
    ...
    def look(self):
        ...
        print 'look observed:', self.Z_gg, 'estimated:', self.toPict(self.X_gate_c, self.X_)
    ...
    def localize(self):
        ...
        dist_w = g_[0]/(gg_[0]-g_[0]+0.0000001)
        dist_h = g_[1]/(gg_[1]-g_[1]+0.0000001)
    ...
    def lookAgain(self):
        ...
        self.localize()
        # mid point 1: (X_gate_c[0]+3, X_gate_c[1], X_gate_c[2]+0.2, pi)
        print 'going to mid point 1'
        self.goY(-(self.X_gate_c[1]-self.X[1]))
        self.goZ(self.X_gate_c[2]+0.2-self.X[2])
        self.goX(-(self.X_gate_c[0]+3-self.X[0]), ('go2poll',))

    def go2Poll(self):
        self.X = np.copy(self.X_)
        # mid point 2: (X_poll_t[0]+3, X_poll_t[1], X_gate_c[2]+0.2, pi)
        print 'going to mid point 2'
        self.goX(-(-6), None)
        self.goY(-(self.X_poll_t[1]-self.X[1]))
        self.goX(-(self.X_poll_t[0]+3-self.X[0]+6), ('goaroundpoll',))

    def goAroundPoll(self):
        # mid point 3: (X_poll_t[0]+3, X_poll_t[1], X_gate_c[2]+0.2, pi)
        print 'going to mid point 3 and 4'
        r = 3
        ...
        self.cmds.append(('halt',))
        # mid point 4: (X_poll_t[0]+3, X_poll_t[1], X_gate_c[2]+0.2, 0)
    ...
    def go2Gate(self):
        self.X = np.copy(self.X_)
        # mid point 5: (X_gate_c[0]-5, X_gate_c[1], X_gate_c[2]+0.2, 0)
    ...
    def goHome(self):
        ...
        self.localize()
        # end point: (X_gate_c[0]+5, X_gate_c[1], X_gate_c[2]+0.2, 0)
        print 'going Home'
        self.goY(self.X_gate_c[1]-self.X[1])

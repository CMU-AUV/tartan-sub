...
class Marker(Task):
    ...
    def find_marker(self, orig, src, template), marker_threshold=0.65):
        ...
        for scale_i, scale in enumerate(scales_l):
            ...
            for pt in zip(*loc[::-1]):
                tl = pt[0]*scales_s[scale_i]
                tr = pt[1]*scales_s[scale_i]
                w = w*scales_s[scale_i]
                h = h*scales_s[scale_i]
                bb = bbox(tl, tr, w, h)
                # Fix this
                BBox.append(bb)
        ...
        for scale_i, scale in enumerate(scales_s):
            ...
            for pt in zip(*loc[::-1]):
                tl = pt[0]*scales_l[scale_i]
                tr = pt[1]*scales_l[scale_i]
                w = w*scales_l[scale_i]
                h = h*scales_l[scale_i]
                bb = bbox(tl, tr, w, h)
                # Fix this
                BBox.append(bb)
